...
test_train_x_data <- rbind(data_x_train, data_x_test)

# Merges y train and y test data to create a 'y' data set
test_train_y_data <- rbind(data_y_train, data_y_test)

# Merges subject train data and subject test data to create 'subject' data set
test_train_subject_data <- rbind(data_subject_train, data_subject_test)
...
names(test_train_x_data) <- features_data[features_data_mean_SD, 2]
...
# correct column name
names(test_train_subject_data) <- "subject"

# bind all the data in a single data set
all_data <- cbind(test_train_x_data, test_train_y_data, test_train_subject_data)
